 ...
 * @param {String} type 类型
 */
function tsTypeTranslate(type) {
  let result = type.trim().toLowerCase();
  switch (result) {
    case '':
    case 'any':
    case '*':
      result = types.tsAnyKeyword();
      break;
    case 'number':
      result = types.tsNumberKeyword();
      break;
    case 'string':
      result = types.tsStringKeyword();
      break;
    case 'bool':
    case 'boolean':
    case 'true':
    case 'false':
      result = types.tsBooleanKeyword();
      break;
    case '{}':
    case 'object':
      result = types.tsObjectKeyword();
      break;
    case '[]':
    case 'array':
    case '...any':
      result = types.tsArrayType(types.tsAnyKeyword());
      break;
    default:
      if (/^\.\.\./.test(result)) {
        result = types.tsArrayType(typeTranslate(result.replace(/^\.\.\./, '')));
      } else {
        result = types.tsTypeReference(types.identifier(result));
      }
      break;
  }
  return result;
 ...
 * @param {Object} comment 注释
 */
function funcComment2TsFuncInfo(comment) {
  if (!comment) {
    return {
      type: '',
      start: 0,
  ...
  funcInfo.params = paramsTypeFlat(funcInfo.params).map((param) => ({
    name: param.name,
    type: tsTypeTranslate(param.type),
  }));
  funcInfo.return = tsTypeTranslate(funcInfo.return);
  return funcInfo;
}

module.exports = {
  funcComment2FuncInfo,
  funcComment2TsFuncInfo,
};
