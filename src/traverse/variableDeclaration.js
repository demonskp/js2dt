 ...
 */
function declarationsDeal(declarations, leadingComment) {
  const resultList = [];
  declarations.forEach((oldDeclarator) => {
    const declarator = oldDeclarator;
    const { init, id } = declarator;
    let newType = types.tsAnyKeyword();
    const newId = types.identifier(id.name);
    if (types.isStringLiteral(init)) {
      newType = types.tsStringKeyword();
    }
    if (types.isNumericLiteral(init)) {
      newType = types.tsNumberKeyword();
    }
    if (types.isBooleanLiteral(init)) {
      newType = types.tsBooleanKeyword();
    }
    if (types.isArrayExpression(init)) {
      newType = types.tsArrayType(types.tsAnyKeyword());
    }
    if (types.isObjectExpression(init)) {
      newType = types.tsTypeLiteral(tsObjectPattern(init.properties));
    }
    if (types.isArrowFunctionExpression(init) || types.isFunctionExpression(init)) {
      newType = getTsFunctionTypeAnnotation(init.async, init.params, leadingComment);
    }
    if (types.isNewExpression(init)) {
      newType = types.tsTypeReference(types.identifier(init.callee.name));
    }
    newId.typeAnnotation = types.tsTypeAnnotation(newType);
    declarator.id = newId;
...
function variableDeclaration(nodePath) {
  ...
  if (types.isVariableDeclaration(node)) {
    ...
    if (declarations[0] && types.isCallExpression(declarations[0].init) && declarations[0].init.callee.name === 'require') {
      ...
      const newDeclaration = declarationsDeal(declarations, leadingComment);
